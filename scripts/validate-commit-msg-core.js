...
const regex = /^(feat|fix|chore|refactor|docs|test|style|build|ci|perf|revert|hotfix|release|migration)\([^)]+\): .+/;
...
function printGlobalError() {
    ...
    console.error(colorize('- exactly as branch name', 'red'));
    console.error('');
    console.error(colorize('DESCRIPTION:', 'red'));
    console.error(colorize('- Short description about what you have done', 'red'));
    console.error('');
    console.error(colorize('Example: fix(change-theming): Some action', 'red'));
}
...
if (!regex.test(message)) {
    ...
    console.info(colorize('Commit message valid <3', 'green'));
}
