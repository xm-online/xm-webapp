...
import { XmTableWidgetConfig } from '@xm-ngx/components/table';
...
export class XmTableMatPaginatorInt extends MatPaginatorIntl implements OnDestroy {
    ...
    constructor(private translate: TranslateService, @Inject('config') private config: XmTableWidgetConfig) {
        super();
    ...
    public getRangeLabel: XmMatPaginatorIntGetRangeLabel = (page: number, pageSize: number, length: number) => {
        ...
        const infinityTotal = this.config?.pageableAndSortable?.infinityTotal;
        const suffix = infinityTotal && length >= infinityTotal ? '...' : '';
        return `${startIndex + 1} - ${endIndex} ${this.OF_LABEL} ${length}${suffix}`;
    };
}
