...
export class DocCommand implements Command {
    ...
    private getExamples(): DocExample[] {
        const modulesPath: string[] = _.flatten(_.map(this.examplesModulePathMask, (themePath) => glob.sync(themePath).map(filePath => filePath.replace(/\\/g, '/'))));
        ...
        for (const modulePath of modulesPath) {
            ...
            const componentsPath: string[] = glob.sync(componentsPathMask).map(filePath => filePath.replace(/\\/g, '/'));
