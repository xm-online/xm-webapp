import { ClassDeclaration, Node, SourceFile, SymbolFlags, SyntaxKind, Type } from 'ts-morph';
...
export function getComponentsSpec(sourceFiles: SourceFile[]): DynamicComponentSpecEntity[] {
    ...
    for (const sourceFile of sourceFiles) {
        ...
        for (const dynamicClass of dynamicClasses) {
            dynamicComponentSpecs.push({
                name: getName(dynamicClass),
                selector: getSelector(dynamicClass),
                compatibles: getCompatibleTypes(dynamicClass),
                configurationSchema: getConfigurationSchema(dynamicClass)
            });
        }
    }
    return dynamicComponentSpecs;
}
